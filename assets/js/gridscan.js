...
import { BloomEffect, ChromaticAberrationEffect, EffectComposer, EffectPass, RenderPass } from 'https://esm.sh/postprocessing@6.34.4?deps=three@0.160.0';
...
function initGridScan() {
  try {
    ...
    if (!container) {
      document.body.insertAdjacentHTML('beforeend', `<div style="position:fixed; top:0; left:0; z-index:9999; background:red; color:white; padding:10px;">GridScan ERROR: container not found</div>`);
      return;
    }

    document.body.insertAdjacentHTML('beforeend', `<div id="gridscan-debug" style="position:fixed; top:0; left:0; z-index:9999; background:blue; color:white; padding:10px;">GridScan Active. Size: ${container.clientWidth}x${container.clientHeight} | <button onclick="this.parentElement.remove()">Dismiss</button></div>`);
  ...
  const uniforms = {
    ...
    uLineStyle: { value: 0 },
    uLineJitter: { value: Math.max(0, Math.min(1, config.lineJitter || 0)) },
  ...
  window.addEventListener('mouseenter', () => { if (leaveTimer) clearTimeout(leaveTimer); });
  ...
  const tick = () => {
    ...
    const tiltSm = smoothDampFloat(tiltCurrent, tiltTarget, tiltVel, smoothTime, maxSpeed, dt);
    tiltCurrent = tiltSm.value;
    tiltVel.v = tiltSm.v;

    const yawSm = smoothDampFloat(yawCurrent, yawTarget, yawVel, smoothTime, maxSpeed, dt);
    ...
    document.body.innerHTML += `<div style="position:fixed; z-index:9999; top:10%; left:10%; width:80%; height:80%; overflow:auto; background:rgba(255,0,0,0.9); color:white; font-family:monospace; padding:30px; font-size:18px;">
      <h2>GridScan Error</h2>
      <pre>${err.message}\n${err.stack}</pre>
    </div>`;
  }
}
...
function pushScan(t) {
    scanStarts.push(t);
    // this logic was tied to the click effect, simplified.
}
